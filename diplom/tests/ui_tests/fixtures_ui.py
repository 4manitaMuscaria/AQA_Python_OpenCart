...
from diplom.pages.login_admin_page import LoginAdmin
...
from diplom.pages.account_register_paige import AccountRegister
from diplom.src.test_data import _TestData
...
@pytest.fixture()
def delete_user(logger, db_session, test_data):

    yield

    logger.info(f"Deleting user {test_data.user_data["email"]} from database...")
    db_session.execute(text("DELETE FROM oc_customer WHERE email = :email"),
                       {"email": test_data.user_data["email"]})
    db_session.commit()


@pytest.fixture()
def delete_address(logger, db_session, test_data):

    yield

    logger.info(f"Deleting address {test_data.address_data["address"]} from database...")
    db_session.execute(text("DELETE FROM oc_address WHERE address_1 = :address"),
...
def delete_file(logger, db_session, test_data):
    ...
    db_session.execute(text("DELETE FROM oc_upload WHERE name = :name"),
                       {"name": "1.jpg"})
    db_session.commit()
